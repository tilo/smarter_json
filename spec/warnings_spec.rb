    ...
    context "acceleration: #{acceleration}" do
      describe "default (no handler)" do
        it "returns the bare value, never a tuple, on input that triggers a lenient fix" do
          ...
          expect(SmarterJSON.process_one('{"a":1,"a":2}', acceleration: acceleration)).to eq({ "a" => 2 })
        end
        ...
        it "does not raise when a lenient fix is applied with no handler set" do
          ...
          expect { SmarterJSON.process_one("[1,,2]", acceleration: acceleration) }.not_to raise_error
        end
      end
      ...
      describe "the handler fires" do
        it "calls on_warning once per warning, passing a SmarterJSON::Warning" do
          ...
          SmarterJSON.process_one("[1,,2]", on_warning: handler, acceleration: acceleration)
          ...
          w2, h2 = collect
          SmarterJSON.process("[1,,2]", on_warning: h2, acceleration: acceleration)
          expect(w2.size).to eq(1)
          expect(w2.first).to be_a(SmarterJSON::Warning)
          expect(w2.first.type).to eq(:empty_slot)
        end

        it "does NOT call on_warning on clean strict JSON" do
          warns, handler = collect
          result = SmarterJSON.process_one('[1, 2, 3]', on_warning: handler, acceleration: acceleration)
          expect(result).to eq([1, 2, 3])
          expect(warns).to be_empty

          w2, h2 = collect
          expect(SmarterJSON.process('[1, 2, 3]', on_warning: h2, acceleration: acceleration)).to eq([[1, 2, 3]])
          expect(w2).to be_empty
        end

        it "fires :empty_slot on a collapsed interior comma" do
          warns, handler = collect
          result = SmarterJSON.process_one("[1,,2]", on_warning: handler, acceleration: acceleration)
          expect(result).to eq([1, 2])
          expect(warns.map(&:type)).to eq([:empty_slot])

          w2, h2 = collect
          expect(SmarterJSON.process("[1,,2]", on_warning: h2, acceleration: acceleration)).to eq([[1, 2]])
          expect(w2.map(&:type)).to eq([:empty_slot])
        end

        it "fires :empty_value on a key with no value (read as null)" do
          warns, handler = collect
          result = SmarterJSON.process_one("{a:}", on_warning: handler, acceleration: acceleration)
          expect(result).to eq({ "a" => nil })
          expect(warns.map(&:type)).to eq([:empty_value])
          expect(warns.first.message).to match(/null/i)

          w2, h2 = collect
          expect(SmarterJSON.process("{a:}", on_warning: h2, acceleration: acceleration)).to eq([{ "a" => nil }])
          expect(w2.map(&:type)).to eq([:empty_value])
        end

        it "fires :duplicate_key on a repeated key (last_wins default)" do
          warns, handler = collect
          result = SmarterJSON.process_one('{"a":1,"a":2}', on_warning: handler, acceleration: acceleration)
          expect(result).to eq({ "a" => 2 })
          expect(warns.map(&:type)).to eq([:duplicate_key])

          w2, h2 = collect
          expect(SmarterJSON.process('{"a":1,"a":2}', on_warning: h2, acceleration: acceleration)).to eq([{ "a" => 2 }])
          expect(w2.map(&:type)).to eq([:duplicate_key])
        end

        it "fires :duplicate_key under duplicate_key: :first_wins too (first value kept)" do
          warns, handler = collect
          result = SmarterJSON.process_one('{"a":1,"a":2}', on_warning: handler, duplicate_key: :first_wins, acceleration: acceleration)
          expect(result).to eq({ "a" => 1 })
          expect(warns.map(&:type)).to eq([:duplicate_key])

          w2, h2 = collect
          expect(SmarterJSON.process('{"a":1,"a":2}', on_warning: h2, duplicate_key: :first_wins, acceleration: acceleration)).to eq([{ "a" => 1 }])
          expect(w2.map(&:type)).to eq([:duplicate_key])
        end
        ...
        it "fires per empty slot for leading + interior, but NOT for a trailing comma" do
          ...
          w2, h2 = collect
          expect(SmarterJSON.process("[,1,,2,]", on_warning: h2, acceleration: acceleration)).to eq([[1, 2]])
          expect(w2.map(&:type)).to eq(%i[empty_slot empty_slot])
        end
        ...
        it "fires empty slots in objects too" do
          warns, handler = collect
          result = SmarterJSON.process_one('{"a":1,,"b":2}', on_warning: handler, acceleration: acceleration)
          expect(result).to eq({ "a" => 1, "b" => 2 })
          expect(warns.map(&:type)).to eq([:empty_slot])

          w2, h2 = collect
          expect(SmarterJSON.process('{"a":1,,"b":2}', on_warning: h2, acceleration: acceleration)).to eq([{ "a" => 1, "b" => 2 }])
          expect(w2.map(&:type)).to eq([:empty_slot])
        end
        ...
        it "fires multiple warnings in document order" do
          ...
          result = SmarterJSON.process_one('{a:1,,b:}', on_warning: handler, acceleration: acceleration)
          expect(result).to eq({ "a" => 1, "b" => nil })
          expect(warns.map(&:type)).to eq(%i[empty_slot empty_value])

          w2, h2 = collect
          expect(SmarterJSON.process('{a:1,,b:}', on_warning: h2, acceleration: acceleration)).to eq([{ "a" => 1, "b" => nil }])
          expect(w2.map(&:type)).to eq(%i[empty_slot empty_value])
        end
      end
      ...
      describe "the Warning object" do
        it "carries type, message, line, col and renders with to_s" do
          ...
          SmarterJSON.process("[1,,2]", on_warning: h2, acceleration: acceleration)
          expect(w2.first.type).to eq(:empty_slot)
          expect(w2.first.line).to be_a(Integer)
          expect(w2.first.col).to be_a(Integer)
          expect(w2.first.to_s).to include("line")
        end
        ...
        it "reports the exact line/col of the collapsed slot" do
          ...
          w2, h2 = collect
          SmarterJSON.process("[1,,2]", on_warning: h2, acceleration: acceleration)
          expect(w2.first.line).to eq(1)
          expect(w2.first.col).to eq(4)
        end

        it "reports the exact line/col of a collapsed slot across CRLF" do
          w2, h2 = collect
          SmarterJSON.process("[1,\r\n,2]", on_warning: h2, acceleration: acceleration)
          expect(w2.first.type).to eq(:empty_slot)
          expect(w2.first.line).to eq(2)
          expect(w2.first.col).to eq(1)
          warns, handler = collect
        ...
        it "reports the exact line/col of an empty value across CRLF" do
          w2, h2 = collect
          SmarterJSON.process("{a:\r\n}", on_warning: h2, acceleration: acceleration)
          expect(w2.first.type).to eq(:empty_value)
          expect(w2.first.line).to eq(2)
          expect(w2.first.col).to eq(1)
          warns, handler = collect
          SmarterJSON.process_one("{a:\r\n}", on_warning: handler, acceleration: acceleration)
          w = warns.first
        ...
        it "reports the duplicate-key warning on the duplicate line across CRLF" do
          w2, h2 = collect
          SmarterJSON.process("{\"a\":1,\r\n\"a\":2}", on_warning: h2, acceleration: acceleration)
          expect(w2.first.type).to eq(:duplicate_key)
          expect(w2.first.line).to eq(2)
          expect(w2.first.col).to be >= 1
          warns, handler = collect
          SmarterJSON.process_one("{\"a\":1,\r\n\"a\":2}", on_warning: handler, acceleration: acceleration)
          w = warns.first
      ...
      describe "every path delivers warnings to the handler" do
        ...
        it "IO, no block" do
          warns, handler = collect
          io = StringIO.new("[1,,2]")
          result = SmarterJSON.process_one(io, on_warning: handler, acceleration: acceleration)
          expect(result).to eq([1, 2])
          expect(warns.map(&:type)).to eq([:empty_slot])

          w2, h2 = collect
          io2 = StringIO.new("[1,,2]")
          expect(SmarterJSON.process(io2, on_warning: h2, acceleration: acceleration)).to eq([[1, 2]])
          expect(w2.map(&:type)).to eq([:empty_slot])
        end
        ...
        it "process_file, no block" do
          ...
          Tempfile.create(["warn", ".json"]) do |f|
            ...
            expect(result).to eq([[1, 2]]) # always-array: one document (the array [1, 2])
          end
          expect(warns.map(&:type)).to eq([:empty_slot])
        end
